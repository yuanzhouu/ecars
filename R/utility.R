...
gcv_lambda_min <- function(X,Y,return="lambda"){
  ls.fit <- glmnet::glmnet(X,Y,family="gaussian")
  df <- ls.fit$df
  df <- df[df<nrow(X)]
...
get_XXX_list <- function(X, alpha){
  ...
  for(i in 1:length(alpha))
  {
    ...
    XXX_list[[i]] <- corpcor::crossprod.powcor.shrink(X1,t(X1),alpha[i],verbose=F)
  }
  return(XXX_list)
} # (RX)^{-a}%*%X
Y_sim <- function(X,inf_index,sigma2,mode,abs=T){
  get_Y3 <- function(X, eta, sigma2){
    lth <- nrow(X)
...
comp_auc <- function(score, inf_index ,p){
  ...
  method_res <- pROC::roc(label_mat,1-as.numeric(score),quiet=T)
  method_cords <- pROC::coords(method_res, "all", ret = c("recall", "precision"),
                         transpose=TRUE)
  lth <- ncol(method_cords)-1
  method_auc <- caTools::trapz(method_cords[1,lth:p],
